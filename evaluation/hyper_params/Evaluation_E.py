...
if __name__=="__main__":
    ...
    for attack in attacks:
        ...
        for i, mal in enumerate(malicious):
            for j, ben in enumerate(benign):
                dir = f'logs/hyper_params/local_epochs/{attack}_{ben}_{mal}'
        ...
        if os.path.exists(f'logs/Original_{attack}_uniform_model'):
            ...
            original_vals = {
                'mal_value': config['local_epochs_malicious_clients'],
                'ben_value': config['local_epochs'],
        ...
        plot_graphs(malicious, benign, results_bda_during[attack], results_bda_after[attack], results_bda_auc[attack],
                    results_mta_during[attack], results_mta_after[attack], results_50_lifespan[attack], r'E_m', r'E_b',
                    f'Local epochs - {attack}')
    ...
    with open('./evaluation/hyper_params/results/best_mal_choices_greedy_E.json', 'w') as f:
    ...
    with open('./evaluation/hyper_params/results/best_mal_choices_balanced_E.json', 'w') as f:
        json.dump(best_choices, f)
    ...
    for allowed_mta_decrease in allowed_mta_decreases:
        ...
        with open(f'./evaluation/hyper_params/results/best_mal_choices_MTA_constrained_{allowed_mta_decrease}_E.json', 'w') as f:
            json.dump(best_choices, f)
