...
from evaluation.hyper_params.utils import extract_lifespans, extract_accuracies, plot_graphs

if __name__=="__main__":
    benign = [0.001, 0.0005, 0.0001]
    malicious = [0.001, 0.0005, 0.0001]
    ...
    for attack in attacks:
        ...
        for i, mal in enumerate(malicious):
            for j, ben in enumerate(benign):
                dir = f'logs/hyper_params/weight_decay/{attack}_{ben}_{mal}'
        ...
        if os.path.exists(f'logs/Original_{attack}_uniform_model'):
            ...
            original_vals = {
                'mal_value': config['adversarial_optimizer']['weight_decay'],
                'ben_value': config['optimizer']['weight_decay'],
        ...
        plot_graphs(malicious, benign, results_bda_during[attack], results_bda_after[attack], results_bda_auc[attack],
                    results_mta_during[attack], results_mta_after[attack], results_50_lifespan[attack], r'\lambda_m',
                    r'\lambda_b', f'Weight Decay - {attack}')
    ...
    with open('./evaluation/hyper_params/results/best_mal_choices_greedy_wd.json', 'w') as f:
    ...
    with open('./evaluation/hyper_params/results/best_mal_choices_balanced_wd.json', 'w') as f:
        json.dump(best_choices, f)
    ...
    for allowed_mta_decrease in allowed_mta_decreases:
        ...
        with open(f'./evaluation/hyper_params/results/best_mal_choices_MTA_constrained_{allowed_mta_decrease}_wd.json', 'w') as f:
            json.dump(best_choices, f)
